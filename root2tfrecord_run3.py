...
path_to_save='/depot/cms/users/kaur214/tfRecords/'
#path_to_save='/work/users/kaur214/'
#path_to_save='/tmp/amkaur/'
#'/eos/purdue/store/user/amkaur/tfrecordNEW-mkfit-125_v2/'#'/data2/legianni/TrainingFrameworkTrackDNN/tfrecord2-benchmark/'
...
for process in newMK: #oldMK:#['TT']:   
        #files = glob(''+paths_all[process]+'/trackingNtuple_1[0-9].*')#[0:20]
        #files = glob(''+paths_all[process]+'/tr*root')#[0:20]

        files = glob(''+paths[process]+'/trackingNtuple_*.*')#[0:20]
        #files = glob(''+paths[process]+'/trackingNtuple_1[0-9].*')#[0:20]
        ...
        for bag in file_bags[:-1]:
            ...
            #print("copying to EOS")
            #os.system("gfal-copy file:////tmp/amkaur/TT/train/"+process+str(i)+".tfrecord davs://eos.cms.rcac.purdue.edu:9000/store/user/amkaur/new_tfrecordNEW-mkfit-125_v2/TT/train/") 
            print("finished %s percent of the job" %str(float(i)/float(n_bag)*100))
            i+=1


#        print("copying to EOS")
#        os.system("gfal-copy file:/tmp/amkaur/TT/train/*.tfrecord davs://eos.cms.rcac.purdue.edu:9000/store/user/amkaur/tfrecordNEW-mkfit-125_v2/TT/train/") 
