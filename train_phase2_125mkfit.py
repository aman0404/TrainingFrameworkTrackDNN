...
path="/work/users/kaur214/"
#path="/tmp/amkaur/"
#path="/eos/purdue/store/user/amkaur/tfrecordNEW-mkfit-125_v2/"
#processes=["TT", "QCD"]
...
def main():
    ...
    for process in processes:
        ...
        if (process.startswith("Soft")):
           ...
           #files=glob.glob("/eos/purdue/store/user/amkaur/tfrecordNEW-mkfit-125_v2/"+process+"/train/*")
           stat=open(path+process+'.txt')
           #stat=open("/eos/purdue/store/user/amkaur/tfrecordNEW-mkfit-125_v2/"+process+'.txt')
        else:
           files=glob.glob(path+process+"/train/*")
           stat=open('/work/users/kaur214/'+process+'.txt')
           #stat=open(path+process+'.txt')
        files=tuple(files)
    ...
    for process in processes:
        ...
        train_size = round(0.90*sample_size)
    ...
    class LossHistory(tf.keras.callbacks.Callback): 
        ...
        def on_epoch_end(self, epoch, logs=None):
            model.save(f'./retrain-spring23TT_full_events/modelTEST-{epoch:02d}')
